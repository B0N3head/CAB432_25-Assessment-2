...
// Strict SPA fallback - only for legitimate client routes (NOT for API routes!)
app.get('*', (req, res) => {
  // NEVER serve SPA for API routes - these should 404 if not found
  if (req.path.startsWith('/api/')) {
    return res.status(404).json({ error: 'API endpoint not found', path: req.path })
  }

  // Also block file extensions and admin/config paths
...
app.listen(port, () => {
  ...
  // Start the render worker if enabled
  if (process.env.ENABLE_RENDER_WORKER === 'true') {
